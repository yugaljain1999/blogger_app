...
async def get_blog_id(blog_id: int, db: db_dependency, current_user: UserBase = Depends(get_current_user)):
    return blogs.get_blog_id(blog_id, db)
...
async def put_blog(id: int, blog:BlogBase, db: db_dependency, current_user: UserBase = Depends(get_current_user)): # here blog is object
    # Get blog by id which needs to be updated
    return blogs.put_blog(id, blog, db)
